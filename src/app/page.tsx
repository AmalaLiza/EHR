...
const PatientField = ({
  ...
  <div
    className={`flex flex-col gap-2.5 ${
      !isLastField ? "md:border-e md:border-border-secondary" : ""
    }`}
  >
    ...
    <span className="text-text-primary" data-testid={label}>
      {value}
    </span>
  </div>
);
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 p-3 text-base">
      ...
      <PatientField
        icon="/calendar.svg"
        label="Date Of Birth"
        value={`${(new Date(patientData.birth_date), "dd MMM yyyy")} (Age:${
          (new Date(), new Date(patientData.birth_date))
        })`}
      />
      <PatientField
...
export default function Home() {
  ...
  return (
    <section className="flex flex-col bg-background-primary rounded-3xl p-6 m-6 mt-16">
